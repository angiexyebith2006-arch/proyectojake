        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6">
                    ...
                    <form action="{{ route('seguimiento.store') }}" method="POST" class="space-y-6" novalidate>
                        @csrf

                        <!-- Card del Formulario -->
                        <div class="card">
                            <div class="card-header">
                                <h3 class="text-lg font-semibold">Formulario de Visita</h3>
                            </div>
                            <div class="card-body p-6">
                                ...
                                <!-- ID Visita -->
                                <div class="mb-4">
                                    <label for="id_visita" class="block text-sm font-medium text-gray-700">ID Visita *</label>
                                    <input type="number" 
                                           name="id_visita" 
                                           id="id_visita" 
                                           value="{{ old('id_visita', $proximoId) }}"
                                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 @error('id_visita') border-red-500 bg-red-50 @enderror"
                                           min="1"
                                           placeholder="ID automático de la visita">
                                    @error('id_visita')
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="text" 
                                           ...
                                           placeholder="Ej: Seguimiento espiritual, visita pastoral">
                                    @error('motivo')
                                        <p class="text-red-500 text-xs mt-1 font-medium">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <!-- Código Usuario -->
                                <div class="mb-4">
                                    <label for="codigo_usuario" class="block text-sm font-medium text-gray-700">Nombre del Responsable *</label>
                                    <select name="codigo_usuario" 
                                        ...
                                        @foreach($usuario as $u)
                                            <option value="{{ $u->id_numero_documento }}" {{ old('codigo_usuario') == $u->id_numero_documento ? 'selected' : '' }}>
                                                {{ $u->nombre }} {{ $u->apellido }} - {{ $u->id_numero_documento }}
                                            </option>
                                        @endforeach
                                    </select>
